...
import { Alert, Dimensions, StyleSheet, Text, View, ScrollView, Image } from 'react-native'
...
import { getCurrentLocation } from '../../navigations/utils/helpers'
import { validateEmail } from '../../navigations/utils/helpers'
import { addDocumentWithoutId, getCurrentUser, uploadImage } from '../../navigations/utils/actions'
...
export default function AddRestaurantForm({ toastRef, setLoading, navigation }) {
    ...
    const validForm = () => {
        ...
        if (!locationRestaurant) {
            ...
            toastRef.current.show("Debes agregar almenos una imagen al restaurante", 3000)
            isValid = false
        }
...
function UploadImage({ toastRef, imageSelected, setImageSelected }) {
    ...
    const imageSelect = async() => {
        ...
        if(!response.status){
            toastRef.current.show("No has seleccionado la imagen.", 3000)
            return
        }
